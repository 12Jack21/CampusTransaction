<template>
	<view class="list">
		<view class="goods-list-box" @tap="listTap(item.id)" v-for="(item, index) in list_data" :key="index">
			<view class="cu-avatar radius" :style="[{ backgroundImage: comImg(item.commodity.img) }]" />
			<view class="goods-info-view">
				<view class="text-cut-2 text-black">{{ item.commodity.name }}</view>
				<view class="goods-info-tools">
					<text class="text-price text-red text-lg">{{ item.price }}</text>
					<text class="text-gray text-sm margin-left-sm">期望价格：￥{{ item.commodity.expectedPrice }}</text>
				</view>
				<view class="action">

...
<img src="./assets/UseCaseDiagram.png" alt="在武大小程序" width="880" height="660" />
...
| 用户       | 中     | 查看/更新个人信息  | 用户在注册后可根据自身情况随时更改个人信息（除身份验证的信息外） |
| 用户       | 中     | 查看别人的个人信息 | 一个用户可查看其他用户的个人信息，除了真正的身份信息以及联系方式（可通过预约确认获得）外 |
| 用户       | 高     | 浏览物品或通告广场 | 用户进入平台后可在界面上浏览各种物品和通告信息               |
| 用户       | 较高   | 商品评论/回复      | 用户可对物品进行评论，询问发布者一些具体的事项，也可以回复别人的评论 |
| 用户       | 高     | 物品预约           | 用户可以对自己感兴趣的物品进行预约，等待发布者的确认         |
| 用户       | 高     | 筛选通告或物品     | 用户可以对繁多的通告或物品根据一些特定条件进行筛选或者搜索，缩小查找范围 |
| 用户       | 高     | 发布通告           | 用户可根据自己的闲置物品，以通告的形式把多种物品包含起来一起发布出去，等待其他用户的预约 |
| 用户       | 较高   | 更新通告           | 用户发布通告后，根据实际场景的变化可以随时更新自己的通告信息 |
| 用户       | 较高   | 更新物品信息       | 对于通告上承载的物品，用户也可以随时更新其信息，如数量、图片等 |
| 用户       | 中     | 关闭通告           | 对于自己发布过的通告，用户可以手动关闭                       |
| 用户       | 高     | 查看预约记录       | 对于自己的所有预约，用户可以去查看任何一个预约的状态，以及进入预约对应的物品中 |
| 用户       | 高     | 查看通告记录       | 用户可以查看自己发布过的所有通告，方便管理记录               |
| 用户       | 高     | 查看交易记录       | 用户可以对失败或者成功的交易进行查阅                         |
| 用户       | 高     | 确认预约           | 有其他用户预约自己的通告后，发布者可以对其进行确认，交易随机生效 |
| 用户       | 中     | 取消预约           | 对物品的预约可能会被系统取消，或者预约者手动取消自己的预约   |
| 用户       | 高     | 评价交易           | 交易完成后，交易双方都能对此次交易进行评价                   |
| 用户       | 较高   | 修改评价           | 用户在手动或默认评价后有一次机会修改评价                     |
| 系统管理员 | 中     | 用户管理           | 系统管理员可以对所有平台上的用户进行统一的管理，包括删除、封禁等 |
| 系统管理员 | 中     | 通告管理           | 管理员可以对所有发布的通告进行合适的操作，如使其失效         |
| 系统管理员 | 高     | 物品管理           | 管理员可对发布的物品进行一定的审核，对于物品里的评论区的评论内容也是如此 |
| 系统管理员 | 低     | 通知管理           | 管理员可以发布一些系统通知，告知所有用户                     |
...
通告发起者可以选择设置一个时间节点，若达到该时间节点还没有请求者，则通告自动关闭，相应的评论功能也关闭，若到达改时间节点已经有请求者，但是发起者没有处理消息，则系统根据预约时间顺序来选择最早预约的请求者进行本次交易，并且通知该请求者预约成功。
...
用户基本信息主要包括用户名、学院、地域、微信/QQ号等信息，当然用户可以选择隐藏诸如微信/QQ号等敏感信息。设置基本信息方便其他用户了解出售者的基本情况，增加交易的成功率。

**（2）交易成功率**

交易成功率的计算方式为：成功交易的次数在总的交易次数中的比例，如果用户在双方达成交易意向后取消交易，则该用户本次交易会被定义为失败交易。考虑到一些用户可能会针对同一样物品发起多个预约，并且需求量只有一个，这种行为会增加出售者的工作量，造成时间的浪费。因此，本程序设置交易成功率这一指标，供出售者参考，起到敦促用户合理使用该程序的效果。

**（3）个人信誉积分**

同上文提到的交易成功率一样，个人信誉积分设立的初衷也是希望规范用户的行为，督促用户合理、正规的使用该程序。个人信誉积分的由来为每次交易过后，交易双方对本次交易的评价，包括购入者对购入物品质量的评价等。个人信誉积分可间接反映出售者的信用、之前出售物品的好坏等信息，可供购买者参考。
...
| 需求分类 | 定义要素       | 需求内容                                                     |
| -------- | -------------- | ------------------------------------------------------------ |
| 应用安全 | 安全机制独立性 | 安全设计和实现应该具有独立性，不能依赖当前主机的<br/>基础安全机制来确保自身和数据不受破环或拒绝服务。 |
| 应用安全 | 安全机制有效性 | 应防止用户绕过其安全控制机制直接尝试访问系统各<br/>项功能。  |
| 应用安全 | 身份认证       | 具备身份认证功能，限制认证信息强度，具有认证失败<br/>后的处置。 |
| 应用安全 | 登录失败处理   | 提供登录失败处理功能，可采取结束会话、限制非法登<br/>录次数和自动退出等措施。 |
| 应用安全 | 登录超时       | 提供登录超时功能，在用户一段时间内无操作的情况<br/>下，应能够自动超时退出。 |
| 应用安全 | 访问权限       | 对不同用户的访问权限进行严格的访问控制，特定权限<br/>的用户只能看到和使用特定的界面及相应的功能。 |
| 应用安全 | 加密技术       | 应采用密码技术保证通信过程中数据的完整性。                   |
| 应用安全 | 输入限制       | 具备输入字符和输入数据的类型、长度和范围检查功<br/>能。      |
| 应用安全 | 连接限制       | 能够对系统的最大并发会话连接数进行限制。                     |
| 应用安全 | 多重登录       | 对单个帐户的多重并发会话进行限制。                           |
| 应用安全 | 身份鉴别       | 能够对所有系统使用者进行有效标识，并且确保使用者<br/>在访问敏感信息之前已经被鉴别。 |
| 应用安全 | 客户端登陆限制 | 允许管理员用户根据需要设置客户端登陆的用户属性，<br/>包括用户登陆的时间段、用户登陆的 IP 地址等。 |
| 应用安全 | 权限初始化     | 用户的权限应该符合最小权限原则。                             |
| 应用安全 | 并发操作保护   | 允许用户对同一系统资源进行不相冲突的访问操作但<br/>应该禁止执行相互矛盾的操作，如两个用户同时修改一<br/>个资源。 |
| 数据安全 | 数据完整性     | 能够检测到网络设备操作系统、主机操作系统、数据库<br/>管理系统以及应急指挥信息管理系统的鉴别信息和重<br/>要业务数据在传输过程中完整性受到破坏。 |
| 数据安全 | 数据保密性     | 对鉴别信息和重要数据采用加密或其他保护措施实现<br/>存储保密性。 |
| 数据安全 | 数据备份和恢复 | 提供对重要信息进行备份和恢复的功能；提供重要网络<br/>设备、通信线路和服务器的硬件冗余；定期进行数据备<br/>份安全恢复实验；建立数据异地容灾；数据库服务器需<br/>考虑多种方式的容灾、容错和备份；数据库服务器应<br/>有独立网段，不允许网内普通用户直接访问数据库服务<br/>器；数据在线保存两年，离线永久保存；系统数据全备<br/>份时间<7 天，增量备份时间<24。 |
| 主机安全 | 集群部署       | 各应用服务器将采用集群部署模式，当集群内的一台或<br/>多台服务器发生故障时，其他服务器自动接管故障服务<br/>器的功能，在故障恢复之后，原故障服务器将自动重新<br/>加入集群。 |
| 主机安全 | 容灾备份       | 对于核心应用，可采用跨机房的容灾备份，将应用软件<br/>及数据备份至异地的机房；对于非关键的应用，可采用<br/>冷备方式将应用软件及数据备份至异地的服务器。容灾<br/>备份保证应急指挥信息管理系统在发生灾难性事故时<br/>可以在最短的时间内恢复平台的运行。 |
| 主机安全 | 负载均衡       | 对于集群部署或主从热备部署的服务器，应急指挥信息<br/>管理系统可采用多种负载均衡策略，可依据各服务器的<br/>压力情况动态调整请求调度策略，极大的提高应急指挥<br/>信息管理系统的整体性能和事务处理能力。 |
| 主机安全 | 防病毒安全     | 部署企业级防病毒软件，以保证系统安全。                       |
